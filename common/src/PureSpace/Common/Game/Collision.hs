...
unitBounds :: (HasPosition s, HasWidth s, HasHeight s) => s -> Rectangle
unitBounds u =
  bounds
    (u ^. position)
    (fromIntegral $ u ^. width)
    (fromIntegral $ u ^. height)
...
createSpatialGrid ::
     (HasPosition s, HasWidth s, HasHeight s)
  => GridSize
  -> GridDivision
  -> [s]
  -> Grid s
createSpatialGrid gs gd@(V2 gdw gdh) units =
  let bs = gs / gd
      ...
      buckets =
        M.fromList
          [ (h, Bucket p $ V.fromList $ unitsInBucket h)
          | x <- [0 .. round gdw]
          , y <- [0 .. round gdh]
          , let p = V2 x y
...
computeRangeWithType ::
     (HasPosition a, HasPosition b)
  => Grid b
  -> a
  -> Rectangle
  -> (Position -> Bool)
...
computeRangeWithType (Grid _ _ bs buckets) x rect isInRange predicate =
  let possibleBuckets = bucketsByRectangle bs rect
      targetBuckets =
        catMaybes $ flip M.lookup buckets . bucketHashId <$> possibleBuckets
