...
runGraphics =
  let go =
        evalStateT
          (runReaderT (runExceptT createGameWindow) defaultClientConfig)
          (initialClientState (defaultClientConfig ^. gameConfig))
      displayResult res =
        case res of
...
runLogic :: IO ()
runLogic = do
  initial <- liftIO $ toNanoSecs <$> getTime MonotonicRaw
